...
Columns: ${analysis.columns.map(c => `${c.name} (${c.dataType}, ${c.missingPercentage}% missing${c.isPII ? ', PII' : ''})`).join('; ')}
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="visible" className="flex flex-col gap-5">
      ...
      {/* ── AI Recommendation ── */}
      <motion.div variants={item} className="rounded-2xl border border-white/[0.08] bg-white/[0.08] overflow-hidden backdrop-blur-md">
        ...
        <div className="p-6">
          ...
          {loadingRec ? (
            ...
            <>
              ...
              <p className="text-[11px] text-gray-400 italic mt-3">Generated by Gemma 4</p>
            </>
          ) : (
      ...
      {onDownloadReport && (
        <motion.div variants={item}>
          <Button
            ...
            variant="outline"
            className="w-full h-11 rounded-xl border-gray-200 text-gray-200 hover:border-[#60a5fa] hover:text-[#60a5fa] hover:bg-blue-500/10 transition-colors gap-2 font-semibold text-sm"
          >
