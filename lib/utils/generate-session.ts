...
export const GENERATE_SESSION = {
  ...
  basename: 'fisibel_generate_basename',
} as const
...
export function clearPersistedGenerateResult() {
  ...
  try {
    ...
    sessionStorage.removeItem(GENERATE_SESSION.basename)
  } catch {
    /* ignore */
  }
}

export function persistGenerateResult(
  rows: unknown[],
  headers: string[],
  ...
  try {
    ...
    sessionStorage.setItem(GENERATE_SESSION.basename, downloadBasename)
  } catch {
...
export function loadPersistedGenerateResult(): {
  ...
  if (typeof window === 'undefined') {
    return { rows: null, headers: null, downloadBasename: null }
  }
  try {
    ...
    if (!rawRows || !rawHeaders) return { rows: null, headers: null, downloadBasename: null }
    ...
    const downloadBasename = sessionStorage.getItem(GENERATE_SESSION.basename)
    return {
      rows: rows as any[],
      headers: headers as string[],
      downloadBasename: downloadBasename?.trim() || null,
    }
  } catch {
    return { rows: null, headers: null, downloadBasename: null }
  }
