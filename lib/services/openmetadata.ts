  ...
  // Created the Fisibel service
  export async function createFisibelService() {
    try {
  ...
  //  Post lineage, WHO/World Bank → Gemma 4 Pipeline → Table
      ...
      const res = await fetch(`${OM_BASE}/api/v1/lineage`, {
        ...
        body: JSON.stringify({
          edge: {
            fromEntity: {
              type: 'pipeline',
              fqn: 'fisibel-pipelines.gemini-generator',
            },
            toEntity: {
              type: 'table',
              id: tableId,
            ...
            lineageDetails: {
              description: `AI-generated using WHO + World Bank statistics for ${domain} domain in Africa. Powered by Gemma 4.`,
              source: 'Manual',
            },
          },
        }),
  ...
  // Fetch all registered Fisibel datasets (for display in UI)
